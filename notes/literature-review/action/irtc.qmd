...
The [generative action models note](../../study-notes/machine-learning/generative-models.qmd#questions) defines noisy samples, clean predictions, and flow time before they enter this algorithm.
...
![Failure modes of naive asynchronous execution and temporal ensemble when consecutive action chunks choose different valid trajectory modes.](../../../images/notes/irtc-2.png){#fig-2 fig-alt="A timeline of two action chunks choosing different paths around an obstacle; naive asynchronous execution jumps between the chunks while temporal ensemble averages them into a path through the obstacle" width="95%"}
...
![How IRTC divides the action horizon into frozen, softly guided, and free regions.](../../../images/notes/irtc-3.png){#fig-3 fig-alt="The old chunk and new chunk overlap across a frozen region and a softly guided region before reaching a free future region" width="100%"}
...
To see how that endpoint error should change the current noisy chunk, define the Jacobian

$$
    \mathbf{J}_\tau
    =
    \frac{\partial\widehat{\mathbf{A}_t^1}}
    {\partial\mathbf{A}_t^\tau}.
$$

::: {.callout-note collapse="true" icon="false" title="Derivation: the endpoint Jacobian and consistency gradient"}

After flattening both chunks, $\mathbf{J}_\tau\in\mathbb{R}^{HM\times HM}$.
Its individual entries are

$$
    [\mathbf{J}_\tau]_{ij}
    =
    \frac{\partial[\widehat{\mathbf{A}_t^1}]_i}
    {\partial[\mathbf{A}_t^\tau]_j}.
$$

The $(i,j)$ entry asks a simple question: if I slightly change coordinate $j$ of the current noisy chunk, how much will coordinate $i$ of the estimated clean chunk change?

At first glance, this may look like $\partial\mathbf{A}/\partial\mathbf{A}$, which would just be the identity matrix.
However, the numerator is $\widehat{\mathbf{A}_t^1}$ rather than $\mathbf{A}_t^\tau$.
It depends on the current sample both directly and through the velocity predicted by the policy, so
...
The second term is the important part.
Since the action model mixes information across timesteps and action dimensions, changing one coordinate of the noisy chunk can affect many coordinates of the final chunk.

Applying the chain rule to the consistency loss gives
...
The weighted residual $\mathbf{W}(\widehat{\mathbf{A}_t^1}-\mathbf{Y})$ says which coordinates of the estimated endpoint are wrong and by how much.
Multiplying by $\mathbf{J}_\tau^\top$ carries that endpoint error backward to the coordinates of the current noisy chunk that caused it.

:::

The negative gradient gives a local descent direction for the differentiable consistency loss when its gradient is nonzero:
...
$$
    ...
    &=-\nabla_{\mathbf{A}_t^\tau}
    \mathcal{L}(\mathbf{A}_t^\tau)\\
    &=\mathbf{J}_\tau^{\!\top}\mathbf W
    \left(\mathbf Y-\widehat{\mathbf A_t^1}\right).
    \end{aligned}
$$

This vector–Jacobian product is computed by backpropagation.
The policy parameters stay fixed during this backward pass.
It tells IRTC how to nudge the current noisy action sample, not how to update the model, so that its estimated clean endpoint agrees better with the old trajectory.

The paper builds on pseudoinverse guidance ($\Pi$GDM).
Suppressing the shared arguments $(\mathbf A_t^\tau,\mathbf o_t,\tau)$, its guided velocity is

$$
    \mathbf v_{\Pi\mathrm{GDM}}
    =\mathbf v_\pi+\alpha_\tau\mathbf g_\tau.
$$

For $0<\tau<1$, the scale is
...
The coefficient and index range are

$$
    c_i=\frac{H-s-i}{H-s-d+1},
$$

with $i\in\{0,\ldots,H-1\}$.
...
## Which part of the chunk is being executed? {#execution-timeline}

The guidance happens while the new chunk is being generated.
For IRTC, the new internal chunk starts from Gaussian noise; the previous actions enter the endpoint-guidance calculation through $\mathbf Y$.
This differs from TRTC's explicitly clean prefix inside the model input.
One inference cycle proceeds in this order:

1. Receive the new observation while the previous chunk continues to execute.
2. Initialise the candidate chunk with noise.
3. At each flow step, estimate its clean endpoint, compare the overlap with $\mathbf Y$, compute the guidance, and update the candidate using the guided velocity.
4. Deliver the completed chunk to the controller and begin executing its still-relevant actions.

There is no need to apply the soft guidance after the completed result becomes available: it has already shaped that result during step 3. [Section 3.3, Algorithm 1](https://arxiv.org/abs/2506.07339).

Measure the new chunk's index $i$ from the observation that starts its inference.
The previous chunk supplies commands while inference runs.
When the new chunk becomes available after $d$ control steps, the controller starts from its still-relevant index $d$; it does not re-execute the elapsed prefix.

For example, take $H=50$, $s=10$, and $d=5$, using the overlap convention above.
The previous chunk has $H-s=40$ actions available from the start of new inference.

| New-chunk indices | Count | Role |
|---|---|---|
| $0\leq i<5$ | $d=5$ | The previous chunk supplies these commands before the new result is ready. |
| $5\leq i<40$ | $H-s-d=35$ | Both chunks cover these future times; soft guidance encourages continuity. |
| $40\leq i<50$ | $s=10$ | Only the new chunk covers these times; guidance weights are zero. |

Thus $H-s-d$ is the length of the revisable overlap, not the starting index of zero padding.
The target's zero-padded region starts at index $H-s$.
Those zeros are ignored by the mask; they are not instructions to stop the robot. [Section 3.2 and Figure 3](https://arxiv.org/abs/2506.07339).

## Why guidance compares an estimated endpoint {#endpoint-guidance}

$\mathbf A_t^\tau$ is the current internal noisy sample.
$\widehat{\mathbf A_t^1}$ is an estimate of where that sample will finish, computed from the current policy velocity.
$\mathbf Y$ comes from the previous planned chunk, and supplies values against which the overlapping endpoint can be compared.
The previous plan is a continuity target, not a new ground-truth demonstration.

The endpoint residual identifies a desired change in clean-action coordinates.
The Jacobian maps that error to a change in the current noisy coordinates.
Adding the residual directly would ignore the sensitivity of the estimated endpoint to those current coordinates.

::: {.callout-note collapse="true" icon="false" title="Worked example: the residual and gradient differ"}

Let a scalar internal sample be $x$, an estimated endpoint be $f(x)=2x$, and its target be $y$.
The consistency loss is
...
The endpoint residual is $y-2x$, but the negative gradient is

$$
  -L'(x)=2(y-2x).
$$

The extra factor is the endpoint sensitivity $f'(x)=2$.
For a vector-valued endpoint, the corresponding operation uses the transpose Jacobian.
Neither formula guarantees that an arbitrary finite update decreases the loss; step size and approximation error still matter.

:::

The method guides intermediate generation rather than averaging two completed action chunks.
That distinction explains why it can encourage continuity without explicitly choosing a midpoint between two strategies.
It is not a proof that every generated trajectory is collision-free.
The paper itself reports imperfect hard-prefix matching and motivates the extra soft overlap guidance in Section 3.2.
...
[ACT](act.qmd#temporal-ensembling-uses-a-common-target-time) explains temporal ensembling of predictions for the same execution time.
[Flow matching](../../study-notes/machine-learning/flow-matching.qmd#flow-matching) explains how a target velocity is available during training but predicted during inference.
[TRTC](trtc.qmd) trains a policy to condition on a clean committed prefix, avoiding IRTC's extra guidance backward pass.
